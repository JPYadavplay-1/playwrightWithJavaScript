...
class AmazonLoginPage extends BasePage {
    constructor(page) {
        super(page);
        this.amazonLogo = this.page.locator('[class="nav-logo-link nav-progressive-attribute"]');
    ...
    async navigate(url = process.env.Amazon_URL) {
        await this.goto(url);
        return this;
    }
...
async yourAccountHeader() {
    return this.YourAccontHdr; // Still OK — async wrapping does occur
}
